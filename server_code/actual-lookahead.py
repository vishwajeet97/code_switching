...
def inference_graph():
	...
	with tf.variable_scope("rnn") as vs:
		for i in range(num_steps):
			...
			output_01, rnn_initial_state_01 = cells(tf.concat([inputs[:,i,:], tf.ones([batch_size, 1])], 1), state_0)
			output_10, rnn_initial_state_10 = cells(tf.concat([inputs[:,i,:], tf.zeros([batch_size, 1])], 1), state_1)
			output_11, rnn_initial_state_11 = cells(tf.concat([inputs[:,i,:], tf.ones([batch_size, 1])], 1), state_1)

			rnn_outputs = tf.stack([output_00, output_10, output_01, output_11], axis=2)
			rnn_initial_states = tf.stack([rnn_initial_state_00, rnn_initial_state_10, rnn_initial_state_01, rnn_initial_state_11], axis=3)
			...
			min_index_state_1 = tf.stack([idx, tf.argmin(entropy[:,2:], axis=1, output_type=tf.int32) + tf.constant(2, shape=[], dtype=tf.int32)], axis=1)
...
if __name__ == '__main__':
	...
	index1 = []
	index2 = []
	model_init_path = "/home/vishwajeet/exp/src/models/cond-rnn_model-4-lookahead-true.ckpt"
