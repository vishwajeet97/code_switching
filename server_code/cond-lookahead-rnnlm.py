...
def inference_graph():
	...
	return input_data, target, keep_prob, loss, rnn_initial_state_0, rnn_initial_state_1, state_0, state_1, entropies
...
if __name__ == '__main__':
	...
	index1 = []
	index2 = []
	model_init_path = "/home/vishwajeet/exp/src/models/cond-rnn_model-4-lookahead.ckpt"
	...
	with tf.device('/gpu:0'):
		...
		while tt < epoch :
			...
			with tf.Session(config=config) as sess:	
				...
				while (step+1)*batch_size*num_steps < len(dev_data):
					...
					state, dev_cost = sess.run([[state_0, state_1], loss],
												feed_dict = {input_data:batch_x,
															target:batch_y, 
		...
		def print_entropies_ppl(perplexity, entropiez, seg_x, index, data, batch_index):

			temp_index = [i+batch_index*num_steps for i in index]
			total_batch = [i[0] for i in data[temp_index]]
			words = [total_batch[i:i+num_steps] for i in range(0,batch_size*num_steps,num_steps)]
			print(type(seg_x), seg_x.shape)
			for b in range(batch_size):

				ppl = perplexity[b,:]
				entropies = entropiez[b,:,:]
				seg = seg_x[b,:,0]
				word = words[b]
		...
		input_data, target, keep_prob, loss, rnn_initial_state_0, rnn_initial_state_1, state_0, state_1, entropies = inference_graph()
		saver = tf.train.Saver(tf.trainable_variables())
		...
		with tf.Session(config=config) as sess:
			...
			while (step+1)*batch_size*num_steps < len(test_data):
				...
				state, test_cost, entopies_print = sess.run([[state_0, state_1], loss, entropies],
											feed_dict = {input_data:batch_x,
														target:batch_y, 
														rnn_initial_state_0: state[0],
														rnn_initial_state_1: state[1],
														keep_prob : 1.0})
				print_entropies_ppl(test_cost, entopies_print, seg_x, index3, test_data, step)
				# total_cost += np.sum(test_cost)/(batch_size * num_steps)
				step += 1
				# print()
