...
def make_wordid_map(data, k):
	...
	counter = collections.Counter([i[0] for i in data])
...
with tf.device('/gpu:0'):
	...
	embedding = tf.get_variable("embedding", [word_vocab_size-unk_word_k+1, rnn_size])
	...
	with tf.Session(config=config) as sess:
		...
		while (step+1)*batch_size*num_steps < len(test_data):
			batch_x, seg_x, batch_y  = get_batch(index2,test_data, wordid_map ,step, batch_size, num_steps)
