...
def make_batch(index,data, wordid_map, batch_index, batch_size, num_steps):
	...
	total_word = copy.deepcopy(total_batch)
	...
	for i in range(0,batch_size*num_steps,num_steps):
		...
		seg_x.append(temp3)
		word_x.append(total_word[i:i+num_steps])
	return (batch_x,np.array(seg_x)[...,np.newaxis],batch_y,word_x)
...
if __name__ == '__main__':
	...
	file2 = "data/SEAME/test.txt"
	file3 = "data/SEAME/segmentation_train.txt"
	file4 = "data/SEAME/segmentation_test.txt"
	...
	with tf.device('/gpu:0'):
		...
		with tf.Session(config=config) as sess:
			...
			while (step+1)*batch_size*num_steps < 2049:#len(test_data):
				entropy_total0 = np.zeros([batch_size, num_steps])
				entropy_total1 = np.zeros([batch_size, num_steps])

				batch_x, seg_x , batch_y, word_x  = get_batch(index,test_data, wordid_map ,step, batch_size, num_steps)
				oracle_seg_x = seg_x[:,:,0]
				entropy_print, loss_o = sess.run([entropy, loss], 
										feed_dict = {input_data:batch_x,
													segmentation_data:seg_x,
													target:batch_y,
													rnn_initial_state: state,
													keep_prob :1.0})
				seg_x = np.zeros([batch_size, num_steps, 1], dtype=np.float32)
				for i in range(0, num_steps):
					...
					for j in range(0, batch_size):
						if entropy_batch1[j][i] > entropy_batch0[j][i]:
							...
							seg_x[j][i][0] = 1.0
					entropy_total0[:,i] = entropy_batch0[:,i]
					entropy_total1[:,i] = entropy_batch1[:,i]

				greedy_seg_x = seg_x[:,:,0]

				state, loss_g = sess.run([final_state, loss],
									feed_dict = {input_data:batch_x,
												segmentation_data:seg_x,
												target:batch_y,
												rnn_initial_state: state,
				...
				# print('\n'.join([' '.join(['{:1}'.format(int(item)) for item in row]) for row in print_array]))
				for row1, row2, row3, row4, row5, row6, row7, row8 in zip(entropy_total0, entropy_total1, entropy_print, oracle_seg_x, word_x, greedy_seg_x, loss_g, loss_o):
					for item in row1:
						print("%.1f"%item, end=" ")
					print("")
					for item in row2:
					...
					for item in row6:
						print(int(item), end="   ")
					print("")
					print("The sentence cost for greedy decoding : %.2f" % row7)
					for item in row3:
						print("%.1f"%item, end=" ")
					print("")
					for item in row4:
						print(item, end="   ")
					print("")
					print("The sentence cost for orcale segmentation : %.2f" % row8)
					for item in row5:
						print(item, end=" ")
					print("")
					print("\n")
