...
file4 = sys.argv[4] # File containing embedding 
...
def get_embedding(filename, wordid_map):
	...
	for line in file:
		...
		if word in wordid_map.keys():
			embedding[wordid_map[word]] = line
...
embedding_trained, embedding_mask_trained = get_embedding(file4, wordid_map)
...
with tf.device('/gpu:0'):
	...
	embedding_mask = tf.constant(embedding_mask_trained)
	embedding_init = tf.constant(embedding_trained)
	...
	outputs, final_state = tf.nn.dynamic_rnn(cells,embedding,initial_state=rnn_initial_state,dtype=tf.float32)
	...
	if not gradient_flow: # Pass gradients back to all the parameters
